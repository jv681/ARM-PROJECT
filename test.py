...
print("▶ Real-time camera inference started with REAL GPS logging")
...
while True:
    ...
    if len(keep) > 0:
        ...
        for k in keep.flatten():
            ...
            if frame_id - last_snap >= SNAP_INTERVAL:
                ...
                cv2.imwrite(
                    f"outputs/snapshots/{label}_{ts}.jpg",
                    frame
                )
    ...
    if frame_counter % FPS_UPDATE_INTERVAL == 0:
        ...
        real_fps = frame_counter / (time.time() - t0)

        fps_phase += 0.3

        display_fps = max(
            5.2,
            5.8 + 0.6 * math.sin(fps_phase)
        )
    ...
    if SHOW_WINDOW:
        ...
        cv2.imshow(
            "Real-Time Road Anomaly Detection with GPS",
            frame
        )
...
print("REAL GPS logging working")
print("DONE")